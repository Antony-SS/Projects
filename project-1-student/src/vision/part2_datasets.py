...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    # Separate images into set A (low pass) and set B (high pass) 
    images_a = []
    images_b = []
...
def get_cutoff_frequencies(path: str) -> List[int]:
    ...
    cutoff_frequencies = np.array(freqs)
...
class HybridImageDataset(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        # Transform to torch array, transform function handles dimension transposing as well apparently
