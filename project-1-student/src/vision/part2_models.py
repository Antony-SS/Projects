...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        one_channel_kernel = np.expand_dims(a = np.expand_dims(a = create_Gaussian_kernel_2D(cutoff_frequency=cutoff_frequency), axis = 0), axis = 0) # sloppy way to get there
        kernel = np.tile(one_channel_kernel, (self.n_channels, 1, 1, 1)) # repeat along channel dim
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        padding = kernel.shape[-1] // 2 # for kernel of 7 // 2 = 3, which is the padding we want, I'm assuming we don't have to deal with even kernel #s
        filtered_image = torch.nn.functional.conv2d(input=x, weight=kernel, padding=padding, groups=self.n_channels)
    ...
    def forward(
        ...
        freq_int = cutoff_frequency[0].item() # get int value from tensor
        kernel = self.get_kernel(freq_int)

        low_frequencies = self.low_pass(x=image1, kernel=kernel)
        high_frequencies = image2 - self.low_pass(x=image2, kernel=kernel)

        hybrid_image = low_frequencies + high_frequencies
        hybrid_image = torch.clamp(hybrid_image, min=0, max=1) # should convert to tensors
